...
under a common action space; and (iii) a risk-sensitive (CVaR) access
formulation whose operating point is provably at least as conservative as
the mean-constrained one. A stylized regret analysis for a tabular surrogate
...
analysis ties to access throughput through the localizer's IoU.
\end{abstract}
...
of detectors has been adapted to TFL, including region-based and YOLO-family
architectures~\cite{prasad2020,li2022hf}, anchor-free keypoint detectors that
resolve the prior-anchor mismatch caused by the diverse aspect ratios of RF
emissions~\cite{zhao2024uav}, and transformer backbones that remain robust
...
li2024hmadrl,bokobza2023ddqsa}. Deep $Q$-networks~\cite{wang2018},
...
access as a \emph{distributional} RL problem~\cite{dabney2018qrdqn,ma2020dsac} with a tail-risk (CVaR)
constraint on PU collision, and show its operating point is at least as
...
subband~\cite{wang2018}. In shared bands such as CBRS and ISM, however, the
...
A detector lineage has followed. Downscaled Faster-RCNN and YOLO-family
...
non-maximum suppression, hard decisions) that are never coupled to an access
...
building on the deep $Q$-network~\cite{mnih2015human} and modern
policy-gradient methods such as PPO~\cite{schulman2017ppo} and soft
actor--critic~\cite{haarnoja2018sac}. Deep $Q$-networks were applied to
...
access~\cite{chang2019distributive,naparstek2018deep}; UCB-style exploration was used in cooperative sensing~\cite{zhang2019deep}; and
...
Constrained MDPs~\cite{altman1999constrained} and their deep solvers
...
grid~\cite{li2024hmadrl}. Cooperative sensing-and-access for industrial IoT
integrates the two via optimization rather than learning~\cite{liu2023ciiot}.
...
We consider a single secondary user (SU) opportunistically accessing a
wideband spectrum of bandwidth $\Wband$ centered at $\fc$, which is
shared by an \emph{unknown and time-varying} number of primary-user (PU)
...
single-resolution time--frequency (TF) representation; we work with the
...
transforms (CWT/CQT) because spectrum monitoring weights all frequencies
equally~\cite{prasad2020}.
...
rather than applying non-maximum suppression to obtain hard boxes; this
...
the true emission, quantified later by the IoU error $\eloc$). The
...
Transmission succeeds (yielding an ACK) iff $\mathrm{Col}(\act)=0$ and the
...
ACK/NACK. The control problem is therefore partially observed. We make the
...
supervised):} wideband I/Q is transformed to a spectrogram and passed to an
...
to accelerate POMDP learning when the observation model is fixed.
...
TF-localization work is that we do \emph{not} apply non-maximum suppression
...
$\bm{z}_t = \mathrm{Enc}(\state_t)$. We adopt an LRU rather than an LSTM or a
Transformer because recent evidence shows point-wise recurrent state-space
...
recursion of a POMDP. We ablate this choice (LRU vs. ConvLSTM) in
Section~\ref{sec:simulation}.
...
\begin{itemize}
\item a value-based agent over a discretized action grid (deep $Q$-network~\cite{hausknecht2015drqn}
on $\bm{z}_t$), suitable when the TF action set is coarsely quantized; and
\item a continuous-control agent (deterministic policy gradient~\cite{lillicrap2016ddpg}) that
...
residual ROC. Temporal-support error does not appear: it is absorbed by the
...
$(1-\Pmd)^{|\mathcal{O}(\act)|}$ (and the FKG inequality gives the same
direction under positive association), yielding \eqref{eq:lem_collision};
...
constraint with a Conditional-Value-at-Risk (CVaR) constraint on this window
variable,
\begin{equation}
\begin{aligned}
...
every fixed $\thr$, $\Zcol^{W}(\thr)$ has a continuous CDF, and lowering
...
larger left endpoint, giving $\thrcvar\ge\thr^{\star}$, i.e.\
\eqref{eq:cvar_conservative}. As the window variable concentrates, the
...
We evaluate six questions: (Q1) does the empirical operating point match the
...
structure of Section~\ref{subsec:access} predicts; and (Q6) does sharper
...
For Q5--Q6 we use a localization-aware variant of the environment in which the
...
follow a logistic-Gaussian observation model; its receiver operating
characteristic is measured empirically (Fig.~\ref{fig:tradeoff}a) and is
...
ablations; the full conv/LRU encoder and a GPU-scale, deep PPO/SAC/QR-DQN
comparison are provided as a PyTorch release for the camera-ready (see
...
measure the mean frequency intersection-over-union (IoU) of its estimated
...
M1 the tail), and Q5/Q6 confirm the localization-to-access chain. Three
limitations bound how far the present evidence reaches.
...
not as evidence they hold on real hardware. Validation on captured wideband
...
\bibitem{sohul2015cbrs}
M.~M. Sohul, M.~Yao, T.~Yang, and J.~H. Reed, ``Spectrum access system for the
  citizen broadband radio service,'' \emph{IEEE Commun. Mag.}, vol.~53, no.~7,
  pp. 18--25, 2015.

\bibitem{prasad2020}
K.~N. R. S.~V. Prasad, K.~B. D'Souza, and V.~K. Bhargava, ``A downscaled
  {Faster RCNN} framework for signal detection and time-frequency localization
  in wideband {RF} systems,'' \emph{IEEE Trans. Wireless Commun.}, vol.~19,
  no.~7, pp. 4847--4862, 2020.

\bibitem{li2022hf}
W.~Li, K.~Wang, L.~You, and Z.~Huang, ``A new deep learning framework for {HF}
  signal detection in wideband spectrogram,'' \emph{IEEE Signal Process.
  Lett.}, vol.~29, pp. 1342--1346, 2022.

\bibitem{zhao2024uav}
R.~Zhao, T.~Li, Y.~Li, Y.~Ruan, and R.~Zhang, ``Anchor-free multi-{UAV}
  detection and classification using spectrogram,'' \emph{IEEE Internet Things
  J.}, vol.~11, no.~3, pp. 5259--5272, 2024.

\bibitem{zhao2025trtfl_ssl}
R.~Zhao, Y.~Ruan, Y.~Li, T.~Li, R.~Zhang, and P.~Xiao, ``A transformer-based
  self-supervised learning framework for robust time-frequency localization in
  concurrent cognitive scenario,'' \emph{IEEE Trans. Wireless Commun.},
  vol.~24, no.~5, pp. 3747--3762, 2025.

\bibitem{zhang2024spectrumtransformer}
W.~Zhang, Y.~Wang, X.~Chen, Z.~Cai, and Z.~Tian, ``Spectrum transformer: An
  attention-based wideband spectrum detector,'' \emph{IEEE Trans. Wireless
  Commun.}, vol.~23, no.~9, pp. 12\,343--12\,353, 2024.

\bibitem{wang2018}
S.~Wang, H.~Liu, P.~H. Gomes, and B.~Krishnamachari, ``Deep reinforcement
  learning for dynamic multichannel access in wireless networks,'' \emph{IEEE
  Trans. Cogn. Commun. Netw.}, vol.~4, no.~2, pp. 257--265, 2018.

\bibitem{zhong2019}
C.~Zhong, Z.~Lu, M.~C. Gursoy, and S.~Velipasalar, ``A deep actor-critic
  reinforcement learning framework for dynamic multichannel access,''
  \emph{IEEE Trans. Cogn. Commun. Netw.}, vol.~5, no.~4, pp. 1125--1139, 2019.

\bibitem{xu2020partial}
Y.~Xu, J.~Yu, and R.~M. Buehrer, ``The application of deep reinforcement
  learning to distributed spectrum access in dynamic heterogeneous environments
  with partial observations,'' \emph{IEEE Trans. Wireless Commun.}, vol.~19,
  no.~7, pp. 4494--4506, 2020.

\bibitem{li2020aggregation}
Y.~Li, W.~Zhang, C.-X. Wang, J.~Sun, and Y.~Liu, ``Deep reinforcement learning
  for dynamic spectrum sensing and aggregation in multi-channel wireless
  networks,'' \emph{IEEE Trans. Cogn. Commun. Netw.}, vol.~6, no.~2, pp.
  464--475, 2020.

\bibitem{li2024hmadrl}
X.~Li, Y.~Zhang, H.~Ding, and Y.~Fang, ``Intelligent spectrum sensing and
  access with partial observation based on hierarchical multi-agent deep
  reinforcement learning,'' \emph{IEEE Trans. Wireless Commun.}, vol.~23,
  no.~4, pp. 3131--3145, 2024.

\bibitem{bokobza2023ddqsa}
Y.~Bokobza, R.~Dabora, and K.~Cohen, ``Deep reinforcement learning for
  simultaneous sensing and channel access in cognitive networks,'' \emph{IEEE
  Trans. Wireless Commun.}, vol.~22, no.~7, pp. 4930--4946, 2023.

\bibitem{hausknecht2015drqn}
M.~Hausknecht and P.~Stone, ``Deep recurrent {Q}-learning for partially
  observable {MDPs},'' \emph{AAAI Fall Symp.}, 2015.

\bibitem{dabney2018qrdqn}
W.~Dabney, M.~Rowland, M.~G. Bellemare, and R.~Munos, ``Distributional
  reinforcement learning with quantile regression,'' \emph{Proc. AAAI Conf.
  Artificial Intelligence}, 2018.

\bibitem{ma2020dsac}
X.~Ma, L.~Xia, Z.~Zhou, J.~Yang, and Q.~Zhao, ``{DSAC}: Distributional soft
  actor-critic for risk-sensitive reinforcement learning,''
  \emph{arXiv:2004.14547}, 2020.

\bibitem{yucek2009survey}
T.~Y\"ucek and H.~Arslan, ``A survey of spectrum sensing algorithms for
  cognitive radio applications,'' \emph{IEEE Commun. Surveys Tuts.}, vol.~11,
  no.~1, pp. 116--130, 2009.

\bibitem{axell2012spectrum}
E.~Axell, G.~Leus, E.~G. Larsson, and H.~V. Poor, ``Spectrum sensing for
  cognitive radio: State-of-the-art and recent advances,'' \emph{IEEE Signal
  Process. Mag.}, vol.~29, no.~3, pp. 101--116, 2012.

\bibitem{arjoune2019comprehensive}
Y.~Arjoune and N.~Kaabouch, ``A comprehensive survey on spectrum sensing in
  cognitive radio networks,'' \emph{Sensors}, vol.~19, no.~1, p. 126, 2019.

\bibitem{oshea2018over}
T.~J. O'Shea, T.~Roy, and T.~C. Clancy, ``Over-the-air deep learning based
  radio signal classification,'' \emph{IEEE J. Sel. Topics Signal Process.},
  vol.~12, no.~1, pp. 168--179, 2018.

\bibitem{oshea2021dataset}
T.~O'Shea \emph{et~al.}, ``A wideband signal recognition dataset,'' in
  \emph{Proc. IEEE SPAWC}, 2021.

\bibitem{vo2020wideband}
T.~D. Vo-Huu and G.~Noubir, ``Wideband signal detection and classification with
  deep learning,'' \emph{IEEE Trans. Cogn. Commun. Netw.}, vol.~6, no.~2, pp.
  533--546, 2020.

\bibitem{prasad2021spectrum}
K.~N. R. S.~V. Prasad and S.~Koley, ``Spectrum sensing and signal localization
  in wideband regimes using deep object detection,'' \emph{IEEE Access},
  vol.~9, pp. 130\,629--130\,642, 2021.
...
K.~He, X.~Chen, S.~Xie, Y.~Li, P.~Doll\'ar, and R.~Girshick, ``Masked
  autoencoders are scalable vision learners,'' in \emph{Proc. IEEE/CVF CVPR},
  2022, pp. 15\,979--15\,988.

\bibitem{mnih2015human}
V.~Mnih, K.~Kavukcuoglu, D.~Silver \emph{et~al.}, ``Human-level control through
  deep reinforcement learning,'' \emph{Nature}, vol. 518, no. 7540, pp.
  529--533, 2015.

\bibitem{schulman2017ppo}
J.~Schulman, F.~Wolski, P.~Dhariwal, A.~Radford, and O.~Klimov, ``Proximal
  policy optimization algorithms,'' in \emph{arXiv:1707.06347}, 2017.
...
T.~Haarnoja, A.~Zhou, P.~Abbeel, and S.~Levine, ``Soft actor-critic: Off-policy
  ...
  \emph{Proc. Int. Conf. Mach. Learn. (ICML)}, 2018, pp. 1861--1870.
...
\bibitem{chang2019distributive}
H.-H. Chang, H.~Song, Y.~Yi, J.~Zhang, H.~He, and L.~Liu, ``Distributive
  dynamic spectrum access through deep reinforcement learning: A reservoir
  computing-based approach,'' \emph{IEEE Internet Things J.}, vol.~6, no.~2,
  pp. 1938--1948, 2019.

\bibitem{naparstek2018deep}
O.~Naparstek and K.~Cohen, ``Deep multi-user reinforcement learning for
  distributed dynamic spectrum access,'' \emph{IEEE Trans. Wireless Commun.},
  vol.~18, no.~1, pp. 310--323, 2019.

\bibitem{zhang2019deep}
Y.~Zhang, P.~Cai, C.~Pan, and S.~Zhang, ``Multi-agent deep reinforcement
  learning-based cooperative spectrum sensing with upper confidence bound
  exploration,'' \emph{IEEE Access}, vol.~7, pp. 118\,898--118\,906, 2019.

\bibitem{tan2020deep}
J.~Tan, Y.-C. Liang, L.~Zhang, and G.~Feng, ``Deep reinforcement learning for
  joint channel selection and power control in {D2D} networks,'' \emph{IEEE
  Trans. Wireless Commun.}, vol.~20, no.~2, pp. 1363--1378, 2021.

\bibitem{lin2019deep}
Y.~Lin, C.~Wang, J.~Wang, and Z.~Dou, ``A novel dynamic spectrum access
  framework based on reinforcement learning for cognitive radio sensor
  networks,'' \emph{Sensors}, vol.~16, no.~10, p. 1675, 2016.

\bibitem{altman1999constrained}
E.~Altman, ``Constrained {Markov} decision processes,'' \emph{Chapman \&
  Hall/CRC}, 1999.

\bibitem{achiam2017cpo}
J.~Achiam, D.~Held, A.~Tamar, and P.~Abbeel, ``Constrained policy
  optimization,'' \emph{Proc. Int. Conf. Mach. Learn. (ICML)}, pp. 22--31,
  2017.

\bibitem{tessler2019reward}
C.~Tessler, D.~J. Mankowitz, and S.~Mannor, ``Reward constrained policy
  optimization,'' \emph{Proc. Int. Conf. Learn. Represent. (ICLR)}, 2019.

\bibitem{chow2017risk}
Y.~Chow, M.~Ghavamzadeh, L.~Janson, and M.~Pavone, ``Risk-constrained
  reinforcement learning with percentile risk criteria,'' \emph{J. Mach. Learn.
  Res.}, vol.~18, no.~1, pp. 6070--6120, 2017.

\bibitem{rockafellar2000cvar}
R.~T. Rockafellar and S.~Uryasev, ``Optimization of conditional
  value-at-risk,'' \emph{Journal of Risk}, vol.~2, pp. 21--42, 2000.

\bibitem{bellemare2017distributional}
M.~G. Bellemare, W.~Dabney, and R.~Munos, ``A distributional perspective on
  reinforcement learning,'' \emph{Proc. Int. Conf. Mach. Learn. (ICML)}, pp.
  449--458, 2017.

\bibitem{liang2008sensing}
Y.-C. Liang, Y.~Zeng, E.~C.~Y. Peh, and A.~T. Hoang, ``Sensing-throughput
  tradeoff for cognitive radio networks,'' \emph{IEEE Trans. Wireless Commun.},
  vol.~7, no.~4, pp. 1326--1337, 2008.

\bibitem{liu2023ciiot}
X.~Liu, M.~Jia, M.~Zhou, B.~Wang, and T.~S. Durrani, ``Integrated cooperative
  spectrum sensing and access control for cognitive industrial {Internet} of
  things,'' \emph{IEEE Internet Things J.}, vol.~10, no.~3, pp. 1887--1896,
  2023.

\bibitem{xing2007dsa}
Y.~Xing, C.~N. Mathur, M.~A. Haleem, R.~Chandramouli, and K.~P. Subbalakshmi,
  ``Dynamic spectrum access with {QoS} and interference temperature
  constraints,'' \emph{IEEE Trans. Mobile Comput.}, vol.~6, no.~4, pp.
  423--433, 2007.

\bibitem{sutton2018rl}
R.~S. Sutton and A.~G. Barto, \emph{Reinforcement Learning: An Introduction},
  2nd~ed.\hskip 1em plus 0.5em minus 0.4em\relax MIT Press, 2018.

\bibitem{lu2024rethinking}
C.~Lu, R.~Shi, Y.~Liu, K.~Hu, S.~S. Du, and H.~Xu, ``Rethinking transformers in
  solving {POMDPs},'' \emph{Proc. Int. Conf. Machine Learning (ICML)}, 2024.

\bibitem{morad2025cae}
S.~Morad \emph{et~al.}, ``A convolution and attention based encoder for
  reinforcement learning under partial observability,''
  \emph{arXiv:2505.23857}, 2025.

\bibitem{lillicrap2016ddpg}
T.~P. Lillicrap \emph{et~al.}, ``Continuous control with deep reinforcement
  learning,'' \emph{Proc. ICLR}, 2016.
...
A.~Garivier and E.~Moulines, ``On upper-confidence bound policies for switching
  bandit problems,'' \emph{Proc. Algorithmic Learning Theory}, pp. 174--188,
  2011.
